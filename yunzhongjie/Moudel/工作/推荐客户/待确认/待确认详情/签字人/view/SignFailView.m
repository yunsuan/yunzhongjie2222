...
- (void)ActionTagBtn:(UIButton *)btn{
    ...
    if (btn.tag == 0) {
        ...
        SinglePickView *view = [[SinglePickView alloc] initWithFrame:self.bounds WithData:[UserModelArchiver unarchive].Configdic[@"18"][@"param"]];
        view.selectedBlock = ^(NSString *MC, NSString *ID) {
            
            self->_nameL.text = [NSString stringWithFormat:@"%@",MC];
            self.agentId = [NSString stringWithFormat:@"%@",ID];
        };
//        WS(weakSelf);
        ...
        if (_nameL.text.length) {
            ...
            if (self.signFailViewBlock) {
                
                self.signFailViewBlock();
                [self removeFromSuperview];
            }
        }
    }
