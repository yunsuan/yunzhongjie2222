...
@interface HMChooseView () <UIPickerViewDataSource,UIPickerViewDelegate>{
    UIView *contentView;
...
- (void)configHourArray {//配置小时数据源数组
    ...
    for (NSInteger i = 0; i < 24; i++) {//加当天的小时数
             NSString *minStr = @"";
            if (i < 10) {
...
- (void)initAppreaence {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (i==0) {
            label.frame = CGRectMake(SCREEN_Width/2 - 40, 260/2-15, 30, 30);
        }else{
            label.frame = CGRectMake(SCREEN_Width/2 + 53, 260/2-15, 30, 30);
        }
...
-(UIView*)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    ...
    UIView *bg = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_Width/2, 30)];
    
    //添加一个label
    UILabel * label = [[UILabel alloc] init];
    
    if (component == 0) {
        label.frame = CGRectMake(SCREEN_Width/2-80, 0, 40, 30);
    }else{
